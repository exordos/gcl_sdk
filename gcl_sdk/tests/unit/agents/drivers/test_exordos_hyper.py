...
class TestResetAndStartMachine:
    """rawstor-vhost exits when its vhost-user client (QEMU) disconnects,
    i.e. exactly when the domain is destroyed - unlike create_machine,
    reset_machine/start_machine don't otherwise touch the vhost units, so
    a plain restart must re-arm them or the domain comes back with a
    dead disk backend.
    """

    def _machine_with_volumes(self, driver):
        machine = pool_base.Machine(
            uuid=sys_uuid.uuid4(),
            project_id=sys_uuid.uuid4(),
            name="vm1",
        ...
        for v in (root_vol, data_vol):
            driver.create_volume(v)
        driver.create_machine(machine, [root_vol, data_vol], [])
        return machine, root_vol, data_vol

    def test_reset_machine_restarts_the_domains_vhost_units(
        self, tmp_path, monkeypatch
    ):
        calls = _no_op_systemctl(monkeypatch)
        driver = _driver(tmp_path)
        monkeypatch.setattr(driver, "_wait_for_socket", lambda socket_path: None)
        machine, root_vol, data_vol = self._machine_with_volumes(driver)
        calls.clear()

        driver.reset_machine(machine)

        started_units = {
            c[-1] for c in calls if c[:3] == ["systemctl", "enable", "--now"]
        }
    ...
    def test_start_machine_restarts_the_domains_vhost_units(
        self, tmp_path, monkeypatch
    ):
        calls = _no_op_systemctl(monkeypatch)
        driver = _driver(tmp_path)
        monkeypatch.setattr(driver, "_wait_for_socket", lambda socket_path: None)
        machine, root_vol, data_vol = self._machine_with_volumes(driver)
        driver.shutdown_machine(machine, force=True)
        calls.clear()

        driver.start_machine(machine)
