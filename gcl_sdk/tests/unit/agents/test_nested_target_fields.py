...
class TestTargetFieldItem:
    ...
    def _item(self, target_fields):
        return storage_base.TargetFieldItem(
            "em_core_network_lb_backend_pools", POOL_UUID, target_fields
        )

    def test_an_item_with_a_shape_can_go_in_a_set(self):
        item = self._item(utils.value_shape(POOL_TARGET))

        assert {item} == {item}

    def test_a_shapeless_item_can_too(self):
        assert {self._item(frozenset(POOL_TARGET.keys()))}

    def test_equal_items_hash_equal_whatever_the_key_order(self):
        """The hash/eq contract, which dict ordering could have broken."""
        one = self._item({"uuid": None, "parent": None})
        other = self._item({"parent": None, "uuid": None})

        assert one == other
        assert hash(one) == hash(other)

    def test_the_shape_tells_two_items_apart(self):
        top_level_only = self._item({"uuid": None, "endpoints": None})
        nested = self._item({"uuid": None, "endpoints": [{"host": None}]})
...
class TestResourceHash:
    def test_shape_makes_the_hashes_match(self):
        ...
        actual = target.replace_value(POOL_ACTUAL, utils.value_shape(POOL_TARGET))
...
class TestTargetFieldsFileStorage:
    ...
    def test_shape_survives_persist_and_load(self, tmp_path):
        ...
        assert item.target_fields == shape
    ...
    def test_update_replaces_the_shape_on_disk(self, tmp_path):
        ...
        storage.create(
            storage_base.TargetFieldItem(
                kind, POOL_UUID, utils.value_shape(POOL_TARGET)
            )
        )
        storage.persist()
        ...
        storage = self._storage(tmp_path, "replace")
        storage.update(storage_base.TargetFieldItem(kind, POOL_UUID, shape))
        storage.persist()

        item = self._storage(tmp_path, "replace").get(kind, POOL_UUID)

        assert item.target_fields == shape
        assert "port" not in item.target_fields["endpoints"][0]
    ...
    def test_update_writes_a_shape_over_an_entry_that_had_none(self, tmp_path):
        ...
        assert isinstance(storage.get(kind, POOL_UUID).target_fields, frozenset)
    ...
    def test_a_file_written_before_shapes_reads_as_shapeless(self, tmp_path):
        ...
        assert isinstance(item.target_fields, frozenset)
        assert item.target_fields == frozenset(POOL_TARGET.keys())
