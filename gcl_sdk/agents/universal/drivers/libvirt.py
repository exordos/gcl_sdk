...
class XMLLibvirtInstance(XMLLibvirtMixin):
    ...
    @classmethod
    def domain_set_boot(
        cls, domain: minidom.Document, boot: pool_base.BootType
    ) -> None:
    ...
    def interface_xml(
        ...
        mac_address = mac or pool_base.Port.generate_mac()
    ...
    def set_boot(self, boot: pool_base.BootType) -> None:
...
class LibvirtPoolDriver(pool_base.AbstractPoolDriver):
    ...
    def _machine2domain_name(self, machine: pool_base.Machine) -> str:
    ...
    def _console_log_path(self, machine: pool_base.Machine) -> str:
    ...
    def _domain2machine(
        ...
        boot = pool_base.BootAlternative.network.value
        for boot_el in element.findall(".//os/boot"):
            boot = boot_el.get("dev")
            if pool_base.BootAlternative.hd0.value.startswith(boot):
                boot = pool_base.BootAlternative.hd0.value
                break
        ...
        for iface in element.findall(".//devices/interface"):
            ...
            ports.append(
                pool_base.Port(
                    ...
                    source=source,
                    status=pool_base.PortStatus.ACTIVE.value,
                )
            )
        ...
        machine = pool_base.Machine(
            ...
            machine_type=pool_base.NodeType.VM.value,
            cores=cores,
            ram=ram,
            image=image,
            # TODO(akremenetsky): Form status from domain state
            status=pool_base.MachineStatus.ACTIVE.value,
    ...
    def _fill_thin_storage_pool(
        self,
        empty_pool: pool_base.ThinStoragePool,
        volumes: tp.Collection[pool_base.MachineVolume],
    ...
    def attach_volume(self, volume: pool_base.MachineVolume) -> None:
        ...
        try:
            ...
            if e.get_error_code() == libvirt.VIR_ERR_NO_STORAGE_VOL:
                raise pool_base.VolumeNotFoundError(volume=volume.uuid)
            raise
    ...
    @dry_run_decorator()
    def attach_port(self, machine: pool_base.Machine, port: pool_base.Port) -> None:
        ...
        try:
            ...
            if e.get_error_code() == libvirt.VIR_ERR_NO_DOMAIN:
                raise pool_base.MachineNotFoundError(machine=machine.uuid)
            raise
        ...
        try:
            ...
            if e.get_error_code() == libvirt.VIR_ERR_OPERATION_INVALID:
                raise pool_base.PortAlreadyAttachedError(
                    port=port.uuid,
                    machine=machine.uuid,
                )
            raise
    ...
    @dry_run_decorator()
    def detach_port(self, machine: pool_base.Machine, port: pool_base.Port) -> None:
        ...
        if target_interface is None:
            raise pool_base.PortNotAttachedError(
                port=port.uuid,
                machine=machine.uuid,
            )
    ...
    def create_machine(
        ...
        ports: tp.Iterable[pool_base.Port],
        ...
        domain.set_boot(pool_base.BootAlternative[machine.boot].boot_type)
    ...
    @dry_run_decorator()
    def delete_machine(
        self, machine: pool_base.Machine, delete_volumes: bool = True
    ) -> None:
        """
    ...
    @dry_run_decorator()
    def set_machine_cores(self, machine: pool_base.Machine, cores: int) -> None:
        """Set machine cores."""
    ...
    @dry_run_decorator()
    def set_machine_ram(self, machine: pool_base.Machine, ram: int) -> None:
        """Set machine ram."""
        ports = self._list_interfaces(machine)
        volumes = self.list_volumes(machine)
    ...
    @dry_run_decorator()
    def shutdown_machine(self, machine: pool_base.Machine, force: bool = False) -> None:
    ...
    @dry_run_decorator()
    def rename_machine(self, machine: pool_base.Machine, name: str) -> None:
        """Rename the machine."""
    ...
    def recreate_machine(
        self,
        machine: pool_base.Machine,
        ports: tp.Optional[tp.Collection[pool_base.Port]] = None,
    ) -> None:
